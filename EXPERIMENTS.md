...
| 5 | `offsegccmiacs_topk2_…` | 4×~24h | 0 | 立论本身（"度量由竞争决定"）从未被直接检验 |

批次的内部结构不是拼盘，是两条轴各取若干点：

- **竞争轴**：1 与 5。5 问"共享几何是否只该在竞争处生效"（限制），1 问"竞争处是否需要
  成对专用的方向"（诊断驱动的加法）。两者都为负，则"竞争条件化"这一立论在论文里必须
  降级表述——这本身是一个必须知道的结论。
...
- 3、5 是**限制**而非加法。8 发结构性加法全负，这两发是这批里唯一不增加容量的干预。
