...
| 5 | `offsegccmiacs_pairdirlate_…` | 4×~25h | 8257 | 对集冻结时刻 4000 → 60000，pairdir 唯一武断超参的对冲 |

批次的内部结构不是拼盘，是两条轴各取若干点：

- **竞争轴**：1 与 5，同一个组件的两个对集冻结时刻（4000 / 60000）。对集一经冻结即永久
  固定，所以"何时统计混淆矩阵"是 PairDir 里最武断的超参；pair 这条线已经因为形式选错被
  误判过一次，两个时刻同批跑才能区分"成对方向没用"和"对选错了"。
...
- 1 vs pair 族（pairwhiten 46.95 / pairraw 46.19）：那一族是**类中心之间的惩罚项**，
  失败的是施加形式；族内 +0.76 恰恰证明白化后的成对方向是对的对象。1 换成像素级、
  竞争门控的反对称 logit 转移，是那个负结果指出的下一个形式，不是同一个实验。
- 2 vs meanboost（46.12）：meanboost 用有界因子给**同一张图算出来的**均值项重新加权，
  不可能引入信息。均值轴关闭的是图内重加权；跨图记忆从未测过。
- 3 是**限制**而非加法，且零参数；8 发结构性加法全负，它是这批里唯一不增加任何容量的
  干预。
