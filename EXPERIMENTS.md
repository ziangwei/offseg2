...
### 7.36 原Route诊断回报与成本追踪修复（2026-09-25）

来源：用户 `C:/Users/21138/Downloads/route_audit_b.zip`，作业5802891，源码
1083f6c6b816837323bb1724bd76e6d05d498962，批次`20260922_183401_927bf8`。
标准ADE2000张、448102043有效像素，原Route best160k权重严格加载，
**mIoU48.4924，复现PASS**；脚本确认注册buffer不变和IoUMetric一致。
实际目录为 `work_dirs/slurm_audits/20260922_183401_927bf8`，输入权重路径同§7.35。
此为验证集只读诊断，不是新训练结果。包内总exit_code=0，但两个FLOPs字段FAILED，
所以不能将整个成本测量写成完整成功。

错误共80980356像素（18.0719%）；错误中GT进入前2/3/5的比例为
53.6862% / 71.0295% / 84.7535%。预测到整图缺席类占错误42.0244%，其余57.9756%。
与§7.18先前诊断计数有微小数值差，mIoU仍复现；不声称逐像素比特一致。

| 分组 | 组内错误率 | 占全部错误 | 错误中GT进入前三 |
|---|---:|---:|---:|
| 3像素边界带 | 37.90% | 24.51% | 72.72% |
| 3像素带外 | 15.45% | 75.49% | 70.48% |
| 5像素边界带 | 33.69% | 31.57% | 72.11% |
| 5像素带外 | 14.89% | 68.43% | 70.53% |
| 小语义连通区域（≤0.1%） | 78.14% | 1.24% | 57.75% |
| 中语义连通区域（0.1%–1%） | 48.67% | 8.28% | 61.29% |
| 大语义连通区域（>1%） | 16.92% | 90.47% | 72.10% |

边界/面积行重叠，不相加；“内部”只是指定半径带外，非可靠语义内部标签。小区域
很难但像素少，mIoU按类平均，不能由1.24%推出小类不值得改。全部错误top3较高
也不保证类别重排可训练，更不能恢复已失败的pair分支。低IoU类并非都只是排序错：
land IoU0.338，错像素GT在前三8.95%；step IoU5.722，对应16.37%；fountain
IoU21.010，precision89.95%/recall21.51%，错像素GT在前三仅0.35%。这些是描述性
结果，不等于证明特征完全缺失、存在某种标注问题或已找到具体有效结构。

首次成本（A100-SXM4-80GB、FP32/TF32关闭、单卡batch1/512crop，原§7.35口径）：

| 模型 | 注册参数 | 同步墙钟均值 | 吞吐 | 原报告峰值分配显存 |
|---|---:|---:|---:|---:|
| OffSeg-B共享权重架构 | 12955400 | 27.173ms | 36.80 img/s | 381.8MiB |
| 原Route-B | 13215968 | 51.448ms | 19.44 img/s | 464.6MiB |

增参260568（约2.01%），首次测得延迟约+89.3%；不能再暗示参数少就几乎无运行成本。
该时延不是完整图像滑窗吞吐，OffSeg也不是独立46.01权重的准确率测试。原Route常驻
分配内存明显高于参数增量，旧脚本同进程顺序测量且首次追踪失败，因此原总峰值显存
暂不作为可靠跨模型差值，待进程隔离复核；没有证据确认是哪一种残留造成。

两模型FLOPs错误均为`Cannot insert a Tensor that requires grad as a constant`。
源码核验EfficientFormer Attention4D.train(False)会在no_grad之外创建普通缓存ab，
缓存来自可学习偏置。已用真实Attention4D类小张量复现该失败，no_grad内eval修复后
追踪通过，输出与state逐值不变。修复只限成本脚本，不改模型源码；每个模型在独立
新进程计时/计数，异常文本截断，追踪FAILED改返回非零。`--cost-only`可跳过已完成
验证，若使用仍披露unsupported ops，未获得修复后的服务器FLOPs，不冒充已补齐。
...
探索从最终评分移至像素特征和本图中心形成，但不能证明这两处就是错误原因。

配置目录`local_configs/offseg2/Base/`，完整文件名：
`offsegccmiacs_protoroute_<kind>_b_r4_responsibility_ade20k_160k-512x512.py`。

| kind / ID | 唯一结构干预 | 新增参数 |
|---|---|---:|
| featurecontext / featurecontext_b_ade | FreqFusion+align之后、Offset之前的轻量双感受野残差 | 34048 |
| centrepool / centrepool_b_ade | Offset生成原始E时使用跨类竞争后的软像素汇聚 | 0 |

**FeatureContext：** 对align得到的256通道特征做1×1降至64、GroupNorm/GELU，
并行逐通道3×3（dilation1/3），相加除sqrt2后GELU，经零初始化64→256投影加回。
不另加预测分支、loss或文本；CCM/IACS/记忆公式均继承，但它们的输入随新特征变化。
相同公共权重时起始前向等于Route，训练后不保证。普通/膨胀深度卷积属于常规算子，
不是独立原创；本项只是小成本性能探索。新增卷积主要MAC约0.56G/512crop（形状
估算，不含归一化/激活，非全模型FLOPs实测），不能称零成本。诊断acc_feature_move
表示新增特征与原特征的平均绝对幅度比，不是精度。

与旧Spatial47.50的区别是作用于原视觉证据，而非候选中心混成的CCM上下文z；
与responseconv46.99区别是发生在类别竞争之前，而非平滑最终加分图；与RCM证据侧
四发区别是仅在align之后做一个窄瓶颈块，没有全局池化级或各层RCM。旧失败仍降低
此方向信心，不声称证据侧从未尝试，也不从SFR/PCE两个负结果比较推出有效性。
风险：重复骨干/FreqFusion已有信息、损伤对齐特征、增加延迟却不涨点。

**CentrePool：** 原OffSeg用coupled分数S逐类softmax空间来汇聚当前类别的图内特征。
新实现先跨类别softmax，再对每类在空间归一化，稳定实现为
`weights = softmax_pixels(log_softmax_classes(S))`，由此产生原始E=W+proj(weights@F)。
原像素偏移路径`softmax_classes(S)`保持原样，其他流程继承。没有硬筛选、GT、可学
温度、额外梯度截断或参数；所有像素仍以软权重参加。新中心会自然改变stage1分数、
支撑和被记忆的内容，不能说后续数值不变，但原写库规则、监督位置、route与IACS
公式不改。与Recollect46.32区别是直接替换原中心的汇聚权重，不向CCM额外注入
描述子；与Route-write/CE区别是未改成融合中心监督/写库权重。不是高斯EM或新发明。
诊断acc_pool_tv衡量汇聚权重相对原式的变化，acc_pool_support是1/Σw²有效像素量。
风险：早期分类竞争尚不可靠，可能使类别中心更难学习；没有零初始化等价承诺。

共同协议：纯视觉ADE20K/150类、S2固定、512crop、160k、4卡×batch4、seed1370346084；
保持原优化器、学习率、数据增强、8k验证、stage1/final CE，从骨干预训练开始，不
加载Route checkpoint微调。2最近普通+1best。每项单独48h作业、动态端口、独立
源码快照/目录。`round3/all/new/structures/default`只排这两项；历史round2菜单仍在。
`results.py round3`读取本轮；失败不自动续调宽度/膨胀率/汇聚温度，不将top-k诊断
当预期增益。若有正结果，再按成本和泛化决定是否替换主模型。

本地检查通过：真实两新头与原Offset/CCM/IACS/Proto代码的CPU梯度、两项CE、
精确优化器恢复、非方形输入、eval记忆冻结；FeatureContext零投影起点/启用记忆后
