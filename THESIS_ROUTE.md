...
> 参数OffSeg12955400→Route13215968（+2.01%）；首次512crop单卡墙钟27.17→51.45ms，
> 少参数不等于低时延。FLOPs追踪因eval缓存带梯度失败，脚本已修复，未获重测结果。
> 原总峰值显存待独立进程复核；不把FAILED写成完整成本成功。详见EXPERIMENTS.md §7.36。
>
> **09-25用户要求训练先行：** 不等待成本重测。Round3两项独立新训练已写：
> FeatureContext（align后/Offset前双感受野窄瓶颈残差，+34048参数）与CentrePool
> （本图原始类别中心的竞争式软汇聚，+0参数）。均独立基于Route48.49，S2/ADE160k，
> 无新loss/外部信息，独立4卡48h。只是待测假设，不宣称诊断已证明有效；见§7.37。
> 提交`python3 tools/slurm/submit.py round3`，成本重跑可选且不自动排队。
>
> **09-22下一步交付：** 原Route-B错误定位与OffSeg/Route成本作业已写，入口
> `python3 tools/slurm/submit_audit_b.py`，独立Slurm4卡/4小时，只读既有权重。
> 验证保持原滑窗；GT仅在预测后分组，统计边界/语义连通区域/逐类错误与top-k。
> 成本统一单卡512 crop前向，默认OffSeg用共享权重仅测架构成本，FLOPs披露漏计。
> 状态code-ready，服务器读数待回传`route_audit_b.zip`；不新增160k训练。见§7.35。
>
> **09-22诊断包已核验：** 七项都完成160k/20次验证。实际配置仅有预期结构差异，
> 所记录环境、seed和训练协议与原Route一致，未发现共同漏训/配置错误。
> 原Route最后四次验证逐点高于所有七项。ModeBank末段第二模式读取率打印为0，
> 两槽曾初始化不等于均有效使用；ContrastMetric/SoftEnergy的IACS mix约.0342/0，
> 其余并非统一变小。文本分支有非零变化，但均无收益。详见EXPERIMENTS.md §7.34。
> last/元数据/曲线/末段标量已补齐，不重复索取；保持原Route，不追加本批变体训练。
>
> **09-22第二批七项best回报：** ModeBank47.43@152k、CentreTilt47.25@160k、
> BlockMetric47.40@160k、ContrastMetric47.91@160k、SoftEnergy47.20@144k、
> TextMetric47.62@152k、TextSubspace47.12@160k，均低于原Route48.49。
> 用户汇总显示七个best文件均存在，尚未核验可加载性、完整日志和实际运行元数据。
> 停止这七个具体实现的扩展，不从ContrastMetric排名最高推出其机制正确。
> 两项文本接入失败不等于所有文本语义无用。主模型仍为原Proto-route48.49。
> 两批十项ADE结构均未超过Route，下一步先核对已有日志/快照的协议、环境与曲线，
> 再决定新结构；不自动安排复跑种子、延长训练或新队列。详见EXPERIMENTS.md §7.33。
>
> **09-18首批Slurm全部完成（用户确认），best汇总：** Relation47.61@144k、
> Dispersion47.59@144k、Recollect46.32@88k，分别低于Route48.49达.88/.90/2.17。
> 三个上下文加法停止扩展。City-B OffSeg79.71@104k补齐本地对照，Route80.69为+0.98；
> Stuff-T Proto42.16@76k，Route42.32相对其+0.16。完整日志/last/实际SHA仍待核验。
> 当时新批次为五项视觉结构（ModeBank/CentreTilt/BlockMetric/ContrastMetric/SoftEnergy）
> 加两项外部文本扩展（TextMetric/TextSubspace），当时config-ready；结果已由09-22更新。
> 用户允许扩大到七项并复用冻结描述向量，明确不跑语义打乱归因或多种子。
> 文本两项单列外部信息协议；本地描述资产缺失，服务器文件存在性待确认，缺失可
> 用原脚本离线生成。round2提交七个独立4卡/48h作业，详情见EXPERIMENTS.md §7.31–7.32。
>
> **09-15历史交付（结果已由09-18更新）：独立Slurm队列，每项4卡/48小时。** Relation、Dispersion、Recollect
...
> route仍是单次运行；offset/logn0的best/last与完成状态待核验。offset **47.22（-0.90）**、
...
> 末 10% 训练记录的平均 proto lambda 分别为 **0.9476 / 0.9278**，两边 n0 都只从 200
> 降到约 192。Stuff 的 IACS mix 末批为 **0.0001**，末段近似静态 ACS，二次修正本身仍非零。
> 因此“proto 两数据集正增量”仍成立，但不能写成“两数据集都证明与动态二阶项协同”。
> `proto_norm` 的增长没有对应中心范数比，lambda 也不是 GT 在场类加权统计，均不能直接
> 归因假阳性或训练退化。细节与日志来源见 EXPERIMENTS.md §7.7。
...
> （+0.92）、两者 47.79（+1.78）、再加记忆 48.12（+2.11）。`0.79+0.92=1.71` 对 `1.78`，**两个组件近似可加、
...
>
> **2026-09-02 的四发给出一条统一结果**：给单图统计量**补充**图外信息为正（proto
> 48.12，+0.33），**收窄**它的输入像素为负（hard 46.97、purity 46.72），在决策侧**增加**
> 成对容量为负（pairdir 46.00，全项目最差的一次加法）。可写的结论是：**单图每类统计量
> 的瓶颈是证据的量，不是证据的纯度。** `reliability = 0.0262` 此前被读作"统计量被不属于
> 该类的像素污染"，purity 与 hard 证伪了这个读法，也回过头解释了 `responsibility`（软后验
> 指派）当初为什么胜过 `spatial`。
>
> **pair 线关闭**（46.19 / 46.95 / 46.00，三发最好也没到 47）。附带一条方法论结论：
> 二类线性可分性探针（"该对在冻结特征里可分 98-100%"）不能用来预测 150 路端到端训练的
> 可用性，今后不得再作为设计依据。
>
> **本轮关闭的轴**：类间漂移惩罚（pairwhiten 46.95 / pairraw 46.19，族为负。此前写的
> "族内 +0.76 证实了 Fisher 的处方"已于 2026-08-30 撤回：两个 arm 都低于对照，在两个
> 失败之间比大小推不出机制结论，见 EXPERIMENTS.md §7.0b）；图级存在性
...
> 学习融合强度）独立基于原 ADE proto；route已核验best=last @160k，后二者仍待日志核验。
> 配置同用 seed 1370346084，route实际seed已确认。route 升为后续设计参照，
> static/local 低于各自原对照，暂不组合。§7.6 旧五发中 Stuff-B 已完成，其余四发已撤销；下文
> 仍提到旧复跑/归因排期的段落不代表当前队列。二阶矩记忆暂缓，不为收束叙事专门排负结果。
>
> **09-07 新回报**：route-write **47.70（-0.79 vs route48.49，暂按best口径）**；
> route-CE约144k时“46多”后由用户主动停止，记为killed/interim，不是满程成绩。
> 两项当前实现不继续，仍保留route48.49；详细阶段、缺口和解释边界见EXPERIMENTS.md §7.12。
>
> **09-08回报：Route-n0=50为46.97（-1.52 vs route48.49，暂按best口径）**。
> 记为owner-reported，best/last和完成状态未核验；停止这个降低初始融合强度的实现。
> 原route48.49、n0初值200仍为对照，不能由此推出200最优或融合越强越好。见§7.14。
>
> **09-09三项回报**：route-n0=800 **47.40（-1.09）**、route-slowmem **47.84（-0.65）**、
> route-fastmem **47.49（-1.00）**，均相对route48.49按best比较。四份日志已核验：全部
> 完成160k/20次验证，三变体last分别47.40/47.38/47.42。保留200/.01，停止扩展参数搜索。
> 末10%训练记录的IACS mix均值：route .2503、n0=800 .3146、slowmem .9866、fastmem
...
>
> **09-10两项回报：Route-classmix 47.58（-0.91），Route-IACS-r8 47.77（-0.72）。**
> 均为owner-reported，暂按best对照Route48.49；best/last、完成状态和实际运行日志待核验。
...
> 端口29504/29505/29506，配置与风险见EXPERIMENTS.md §7.25；T结果见§7.27。
>
...
> 实际seed/SHA、此前best及末段统计待核验。详见§7.26；当前待回报项以§7.27为准。

> **09-13收到Stuff-T Route截图：42.32，best=last @80k。** 截图显示09-12 23:21:43
...
09-22诊断核验：本批七项均跑满；与原Route逐字段对比未见预期改动之外的配置差异，
已记录环境一致。最后四次验证原Route均领先。ModeBank末段第二槽没有可见读取，
是具体实现未兑现双模式使用的证据；其他分支非零但无收益。不能由标量相关性推因果。
详见§7.34；此更新补齐此前日志缺口，当前没有新增训练队列。

09-22：第二批七项均低于Route48.49，最高ContrastMetric47.91仍低0.58；两项文本
扩展47.62/47.12也无增益。停止七个当前实现的调参与组合，保留原Route；未证明
失败原因或整个方法族无效。best/iter和文件存在性来自用户汇总，完整运行核验待补。
下一步优先只读核验两批与原Route的可比性及训练曲线，不自动新增训练，见§7.33。

09-18：Relation47.61、Dispersion47.59、Recollect46.32，均为已完成训练的用户best
汇总，停止这三个实现。当前最高仍是原Route48.49。新七项是独立探索，不组合失败
结构；五个视觉结构不增加外部信息，两个冻结文本扩展经用户授权另列，当时均未测，
结果已由09-22更新。
...
Cityscapes-B现有本地OffSeg best79.71@104k，Route best80.69为+0.98；实际运行
元数据及last日志仍缺，见§7.31。单次增量不等于跨种子稳定性。

Stuff-T链条为本地OffSeg41.66 → 无记忆42.08 → **Route42.32**，截图确认80k刷新best。
Stuff两档完整Route相对本地OffSeg的增量分别为T **+0.66**、B **+0.49**；相对无记忆
分别为+0.24/+0.42。09-18补齐T原Proto42.16@76k，路由相对它+0.16；不能将
相对无记忆的+0.24当路由单独增益。B的+0.16为末次权重口径、T为best，不能仅因
数字相同推断容量规律。两档均为单次证据，历史T对照seed仍未核验。
...
| proto + 融合中心重算 CCM 候选权重（route，09-09日志核验） | **48.49** | **+0.37，best口径** |

整条表均为单次读数，proto 的原始日志已于 2026-09-04 核验，没有 repeated seeds。
route已核验best=last=48.49 @160k、20次验证，仍为单次运行；其stage-1 CE/support/写库masks未改，
...
09-07新增：route-write47.70低于route48.49；route-CE在约144k时46多被主动停止，
满程上限未知。这两项没有带来可用收益，当前保留原写库与原始中心监督。
09-08的Route-n0=50回报46.97，同样低于route48.49，停止当前较弱初始融合。
这一结果不能证明更强融合必然更好；09-09已收到并核验完整route日志。
09-09的更强初始融合/慢EMA/快EMA分别回报47.40/47.84/47.49，均未超过route48.49。
本轮参数探索结束，保留200/.01；不从三个负结果间的排序推断正确时间尺度。
四份日志最后5次验证均逐点以route最高，差距并非仅由best挑选造成。慢/快更新的mix
在32k已分别达到.7651/.0022，末段为.9866/日志精度0；原型范数末段均值约12.98/72.04，
route约23.44。下一步关注相对尺度和残差强度的只读诊断，不由这些相关性直接推出修复方案。
...
测量与实际运行日志核验（City配对读数已为79.71→80.69）。当前可以称"达到目标的主模型"，不能称"稳定提升"或
"同参数档 SOTA"。
...
| 当前最高 proto-route | 48.49 | 日志核验：best=last @160k，20次验证，seed1370346084；单次运行 |
...
9. 纯视觉主线不引入CLIP等外部模型。09-18用户授权复用离线冻结描述向量作两项
   独立文本扩展；训练/推理不运行文本编码器，但它们仍使用外部语义信息，必须与
   纯视觉结果单列，不能隐去CLIP来源或声称已隔离语义贡献。
...
最终读数：Signature-RGE **46.71**，Bipolar-RGE **46.61**；分别比 RGE 47.56 低
...
2026-09-22诊断更新：§7.34已补齐第二批七项运行记录的SHA/环境/seed/文本资产hash、
实际配置、完整20次验证曲线、last、末段40条标量与批次路径；七项完成状态已由
160k训练和终点验证确认。未核验权重可加载性和Slurm退出码；没有ModeBank全程
槽位轨迹。下段同日早先缺口以此更新为准，不再重复索取包内已有信息。

2026-09-22：第二批七项best/iter与best文件存在性已由用户汇总补齐，见§7.33。
仍缺实际run.json/SHA/环境/seed、完整验证曲线、last、末段标量与真实文本资产hash。
四项best@160k已到配置终点验证，另三项不能仅由早于160k的best判定是否完成。
未安排后续训练；先查已有证据，不将未测原因写成新结构动机，不恢复被拒绝的控制。

2026-09-18：首批五项已由用户确认完成并提供best/iter，见§7.31。City-B本地基线
和Stuff-T原Proto已补齐；仍缺完整日志、last、实际seed/SHA与批次路径。新七项
config-ready，未在本机向LRZ提交。真实描述向量未在本地取得，文本检查使用显式
合成夹具，服务器提交前必须有正确资产。用户拒绝语义打乱/去文本归因对照及多种子，
这些限制只作为未测范围，不自动恢复为排期。新模型成本待实测。
...
§7.9–7.10。route相关缺口以09-09更新为准；以下旧排期不恢复用户已撤销的复跑/归因实验。
