...
>
> **当前三槽（config-ready）**：route-n0=800、route-slowmem（EMA新信息比例.001）、
> route-fastmem（.1）。三个独立对照route48.49，后两项n0仍200，前一项EMA仍.01。
> 原写库对象与等权汇总、监督、完整IACS均保留；检查使用强度和时间尺度，不叠加新模块。
> 这是一批粗尺度参数探索，无训练结果，也无证据保证其中某项会胜出。见EXPERIMENTS.md §7.15。
>
...
当前三槽只探索更强初始融合和快/慢两档EMA；不将参数搜索包装成新方法贡献。
