...
>
> **当前五个训练槽位**（EXPERIMENTS.md §7.6，均为 `config-ready`）：全部围绕 proto——
> 归因（固定 lambda、纯 OffSeg 上的记忆）、复现（seed 2026 配对差）、泛化（Stuff T/B，
> 两者本环境配对基线均已存在）。Cityscapes 与训练配方轴暂缓到 proto 定稿之后。
>
