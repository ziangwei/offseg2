...
> 方向**：一是把 §3 的误差诊断当作设计依据（成对判别方向覆盖 33.7% 的错误，且这些对
> 在冻结特征里线性可分 98-100%），二是把干预从"增加容量"换成"限制"和"补充信息"
> （按纯度净化统计量、只用赢下的像素、按支撑度收缩、跨图类别记忆）。
>
...
>
> **当前五个训练槽位**（EXPERIMENTS.md §7.4，均为 `config-ready`）：全部在方法侧，
> 分两条轴——竞争轴（pairdir 一发）与证据质量轴（proto、purity、support）。这是本项目
> 第一批以 §3 诊断探针表为设计依据、而不是以几何直觉为依据的实验。Cityscapes 两发与
> 训练配方两发暂缓到方法定稿之后。
>
