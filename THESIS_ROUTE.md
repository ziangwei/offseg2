...
>
> **当前单槽：Route-n0=50（config-ready）**。只把route的n0初值200降到50，沿用
> 原softplus、等权EMA写库、原stage-1 CE与完整IACS。它是已有效模型的融合强度探索，
> 50不是已知最优值；旧proto的n0探索范围窄是依据，route融合过强并未被证明。见§7.13。
>
...
满程上限未知。这两项没有带来可用收益，当前保留原写库与原始中心监督。单槽转向
Route-n0=50，检查较弱的初始记忆融合；不将此超参数探索包装成新的方法贡献。
