...
>
> **当前第一训练槽（09-09，config-ready）：Route-classmix。** 基于48.49，只把IACS
> 共用mix改为每类一个，初值均保持.10，净增149参数；记忆200/.01、路由、全候选和两项
> CE不变，从原骨干预训练初始化训练160k。它检验逐类折中的假设，诊断不保证其有效；
> 旧top-3+classmix45.92是多变量组合，不能单独归因。配置与检查见EXPERIMENTS.md §7.19。
>
> **新增第二槽（09-09，config-ready）：Route-IACS-r8。** 独立基于原48.49，残差rank4→8，
> 保留共享mix，不叠加classmix；净增153600参数，计算量增加。旧无记忆IACS-r8曾为46.76
> （-0.65），这次仅在责任度/记忆/路由改变后的工作点重检容量，没有容量瓶颈已证实的说法。
> 原scale与mix初值不变，起点不与r4恒等。仍从原骨干初始化训练160k，见EXPERIMENTS.md §7.20。
>
