# -*- coding: utf-8 -*-
"""PARSeg-LDR: language-described reranking.

Principle (text as EVIDENCE): the proven +0.4 core (LCR: per-pixel top-k
candidate reranking) keeps everything it has -- learned class embedding,
probability features, losses -- and additionally receives, per candidate,
"does this pixel match any of the candidate's described attributes":
    desc_max(c)  = max_k  cos(pixel_feat, P(desc_{c,k}))
    desc_mean(c) = mean_k cos(pixel_feat, P(desc_{c,k}))
    desc_gap(c)  = desc_max(top1) - desc_max(c)
Three scalars appended to the scorer's input. Nothing is replaced, nothing
is constrained: if the text evidence is useless the MLP learns to ignore it
(the LTA lesson inverted -- augment, never constrain). The name-cone issue
does not apply: descriptions enter as match SCORES against per-class SETS
(max over K), not as class-identity vectors.

Inference uses only the frozen description buffer (constants in the ckpt);
no text model anywhere. Delta path is zero-initialized: the model starts
training as plain PARSeg3, exactly like LCR v1.

args (new): ldr_desc_path='assets/text_anchors/ade20k_clip_vitb32_desc6.pt'
(all lcr_* args identical to LCR v1)
"""
import math
import os
...
def load_desc_asset(path, num_classes):
    """Load [C, K, E] frozen description embeddings, per-desc L2-normalized."""
    ...
    if emb.dim() != 3 or emb.shape[0] != num_classes:
        raise ValueError(
            ...
            f'num_classes={num_classes} (need [C, K, E]); path={p}')
    return F.normalize(emb, p=2, dim=-1)


class DescribedCandidateRelation(nn.Module):
    """LCR v1's relation scorer + per-candidate description-match evidence."""

    def __init__(self, in_channels, num_classes, desc_path,
                 relation_dim=64, topk=5, hidden=128,
        ...
        self.feat_proj = ConvModule(
            in_channels, relation_dim, 1,
            conv_cfg=conv_cfg, norm_cfg=norm_cfg, act_cfg=act_cfg)
        self.class_embed = nn.Embedding(num_classes, relation_dim)

        desc = load_desc_asset(desc_path, num_classes)         # [C, K, E]
        self.register_buffer('desc', desc)
        self.desc_proj = nn.Linear(desc.shape[-1], relation_dim, bias=False)
        nn.init.normal_(self.desc_proj.weight, std=0.02)

        # v1 input (3*D + 2 scalars) + 3 description-evidence scalars
        self.score_mlp = nn.Sequential(
            nn.Linear(relation_dim * 3 + 5, hidden),
            nn.GELU(),
    ...
    def forward(self, feat_aligned, raw_base_logits):
        ...
        topk_idx = raw_detached.topk(k=topk, dim=1).indices     # [B,Kc,H,W]
        topk_vals = raw_detached.gather(1, topk_idx)
        topk_prob = F.softmax(raw_detached, dim=1).gather(1, topk_idx)

        feat = self.feat_proj(feat_aligned)
        feat = F.normalize(feat, p=2, dim=1, eps=1e-6)          # [B,D,H,W]

        # ---- description-match maps against ALL classes, then gather ----
        t = F.normalize(self.desc_proj(self.desc), p=2, dim=-1) # [C,K,D]
        match = torch.einsum('bdhw,ckd->bckhw', feat, t)        # [B,C,K,H,W]
        desc_max_all = match.max(dim=2).values                  # [B,C,H,W]
        desc_mean_all = match.mean(dim=2)                       # [B,C,H,W]
        desc_max = desc_max_all.gather(1, topk_idx)             # [B,Kc,H,W]
        desc_mean = desc_mean_all.gather(1, topk_idx)
        desc_gap = desc_max[:, :1] - desc_max                   # vs top-1
        ...
        relation_input = torch.cat(
            [
                ...
                desc_max.permute(0, 2, 3, 1).contiguous().unsqueeze(-1),
                desc_mean.permute(0, 2, 3, 1).contiguous().unsqueeze(-1),
                desc_gap.permute(0, 2, 3, 1).contiguous().unsqueeze(-1),
            ],
            dim=-1,
        )
...
class PARSegLDR(PARSeg3):
    """PARSeg3 + description-evidence candidate reranking (LCR v1 skeleton)."""
    ...
    def __init__(self, in_channels, new_channels, num_classes, cls_attributes,
        ...
        self.args = args or {}
        self.lcr = DescribedCandidateRelation(
    ...
    def _forward_from_aligned(self, feat_aligned):
        ...
        if fusion_mode == 'AGCF':
            ...
            raise ValueError(f'Unsupported fusion_mode for PARSegLDR: {fusion_mode}')
    ...
    # ---- losses: byte-identical to LCR v1 ----
    ...
    def _small_label(self, seg_label, size):
        return F.interpolate(
            seg_label.unsqueeze(1).float(), size=size,
    ...
    def loss_by_feat(self, seg_logits, batch_data_samples):
        ...
        if auxw > 0:
            ...
            losses['loss_lcr_aux'] = self.loss_decode(
                relation_logits, seg_label,
                ignore_index=self.ignore_index) * auxw
