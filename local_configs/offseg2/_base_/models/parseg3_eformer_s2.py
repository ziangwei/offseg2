...
model = dict(
    ...
    decode_head=dict(
        ...
        args=dict(
            ...
            refinementw=1.0,       # 精修头 CE 权重
            fusionw=1.0,           # 融合输出 CE 权重
            refinement_focusw=0.25,# base-error-focused CE 权重(他没写,走代码默认 0.25)
            focus_err_weight=1.0,  # 同上,默认
            focus_unc_weight=0.5,  # 同上,默认
            focus_class_balance=True,
            intra_div=2.0,         # 属性去相关权重
            proto_residual_scale=1.0,  # 原型校准强度(满残差,较激进)
            proto_topk_div=64,
        )),
    train_cfg=dict(),
