# Shared residual-direction dictionary on COCO-Stuff164K-T (EfficientFormerV2-S1).
#
# Partner of the Base run, which read 44.33 -> 44.46 (paired gain +0.07 ->
# +0.20).  T carries the same 171 classes and the same 80k schedule on a
# SMALLER backbone, so it asks whether basis sharing helps because per-class
# data is scarce -- which would repeat here -- or because of something
# specific to the B-scale model.  T's paired baseline is 41.66 and the
# private-basis model is 42.08 (+0.42), the project's best cross-dataset
# cell, so this also tests whether sharing still pays where the private
# bases were already doing comparatively well.
#
...
work_dir = \
    './work_dirs/offsegccmiacs_dict64_r4_responsibility_t_stuff164k_80k-512x512'
